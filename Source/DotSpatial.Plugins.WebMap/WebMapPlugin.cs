...
namespace DotSpatial.Plugins.WebMap
{
  ...
  public class WebMapPlugin : Extension
  {
    ...
    /// <summary>
    /// Gets name of current provider (if any)
    /// </summary>
    public string CurrentProviderName => CurrentProvider?.Name;
    ...
    /// <summary>
    /// Programmatically select the service used
    /// </summary>
    /// <param name="theServiceName">
    /// Name of the service to use.
    /// If there is no service with this name, the service is not changed.
    /// Using case insensitive, culture invariant string comparison
    /// </param>
    public void SelectService(string theServiceName)
    {
      if (!string.IsNullOrWhiteSpace(theServiceName))
      {
        ServiceProvider theServiceProvider = _serviceDropDown.Items.Cast<ServiceProvider>()
          .Where(x => string.Compare(x.Name, theServiceName, true, CultureInfo.InvariantCulture) == 0)
          .FirstOrDefault();

        if (theServiceProvider != null)
        {
          _serviceDropDown.SelectedItem = theServiceProvider;
        }
      }
    }

    /// <summary>
    /// Returns a list of strings containing all the service names available
    /// </summary>
    /// <returns></returns>
    public List<string> ServiceNames()
		{
      return _serviceDropDown.Items.Cast<ServiceProvider>()
        .Select(x => x.Name)
        .ToList();
		}
    ...
    private void AddServiceDropDown(IHeaderControl header)
    {
      ...
      // "Other" provider
      // For BENSPLASH: Removing this as an option
