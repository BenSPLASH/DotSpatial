...
namespace DotSpatial.Plugins.TableEditor
{
  ...
  public class TableEditorPlugin : Extension
  {
    ...
    private void AddContextMenuItems(ILayer addedLayer)
    {
      ...
      // Find the first layer with sub-menu items and insert this before it otherwise just add it to the end.
      SymbologyMenuItem insertBeforeMnu = addedLayer.ContextMenuItems.FirstOrDefault(x => (x.MenuItems.Count > 0));
      if (insertBeforeMnu == null)
      {
        addedLayer.ContextMenuItems.Add(menuItem);
      }
      else
      {
        addedLayer.ContextMenuItems.Insert(addedLayer.ContextMenuItems.IndexOf(insertBeforeMnu), menuItem);
      }
    }
